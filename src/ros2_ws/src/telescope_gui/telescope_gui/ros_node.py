"""ROS2 integration for the telescope GUI app.

The PyQt5 GUI never talks to the camera hardware directly. A separate ROS2 camera
node publishes ``sensor_msgs/msg/Image`` and exposes ``Gain`` / ``Exposure`` /
``Offset`` parameters. This module:
...
All ``rclpy`` imports are done lazily so the GUI modules can be imported (and CV
unit tests can run) on a machine without ROS2 installed.
"""
