...
Performance notes for the Raspberry Pi:
* ``QImage(buffer, ...)`` does NOT copy its backing memory. We keep a reference to
  the contiguous NumPy buffer (``self._buf``) for as long as the QImage/QPixmap is
  alive, otherwise the preview would render freed memory (garbage/crash).
* Scaling is done with ``Qt.FastTransformation`` for a smooth live feed; the
  capture path saves the full-resolution frame separately.
"""
...
class PreviewWidget(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._buf: np.ndarray | None = None   # keeps QImage memory alive
        self._frame: np.ndarray | None = None  # last displayed frame (for capture)
        self._pixmap: QPixmap | None = None
        self._focus: float | None = None
        self._night = False
        self._flash = 0  # frames remaining for capture confirmation flash

        # Floating circular capture button, repositioned in resizeEvent.
    ...
    def update_frame(self, frame: np.ndarray) -> None:
        """Display a uint8 HxWx3 RGB frame."""
        self._frame = frame
    ...
    def current_frame(self) -> np.ndarray | None:
        """The last displayed frame (post-stretch), for saving to disk."""
        return self._frame

    def flash_confirmation(self) -> None:
        """Briefly flash the border to confirm a capture was saved."""
        self._flash = 8
        self.update()
    ...
    def _layout_capture_button(self) -> None:
        """Re-assert the capture button's size and centre it along the bottom.

        Also called after a theme change: swapping the app stylesheet re-polishes
        the button, and re-applying the fixed size here keeps a future stylesheet
        edit from quietly resizing it out from under ``move()``.
        """
        size = CAPTURE_SIZE
        margin = 18
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        if self._pixmap is not None:
            ...
            if self._night:
                # Red multiply so the image itself protects night vision.
