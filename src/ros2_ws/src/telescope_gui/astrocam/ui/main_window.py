...
Layout (1024x600 touchscreen):

    +-------------------------------------------------------+
    | top bar: brand | status | temp | fps | mode | night  |
    +------+---------------------------------------+--------+
    | rail |            PreviewWidget              | panel  |
    |      |                                       | stack  |
    +------+---------------------------------------+--------+

The icon rail selects which panel is shown; tapping the active tool again
collapses the panel so the preview expands to the full width. All ROS frames
arrive on the Qt main thread via queued signals from ``RosBridge``.
"""
...
# Analysis (histogram/focus/stats) is expensive on a Pi: run it on a downsampled
# view and no more than this often, while the preview keeps updating every frame.
ANALYSIS_INTERVAL_MS = 250
ANALYSIS_DOWNSAMPLE = 2

IMAGES_DIR = Path(__file__).resolve().parents[2] / "images"

# The Raspberry Pi 7" touchscreen, which the layout is tuned for.
DESIGN_SIZE = (1024, 600)
# Floor for desktop testing: the rail (76) and panel (280) are fixed width and the
# preview will not go below 320, so anything narrower just clips.
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("أبصار — AstroCam")
        # 1024x600 is the Pi touchscreen's exact resolution, so it is the design
        # size and the default — but only the default. A fixed size here would
        # also veto showFullScreen() and maximising, which is why this is a
        # resize() plus a floor rather than setFixedSize().
        ...
        # The app-wide stylesheet is set in main.py; here we only sync the
        # preview's night tint and the toggle button to the restored state.
    ...
    def _build_topbar(self) -> QFrame:
        ...
        # Touch-reachable fullscreen control: the rig has no keyboard, so F11
        # alone would strand anyone who left fullscreen on the touchscreen.
    ...
    def _build_rail(self) -> QFrame:
        ...
        for tool, icon, label in (
            ...
            btn = self._make_rail_button(f"{icon}\n{label}")
        ...
        # Future-feature actions: visible but disabled.
        self._moon_btn = self._make_rail_button("🌙\nMoon")
        self._moon_btn.setEnabled(False)
        self._moon_btn.setToolTip("Go to Moon — coming soon")
        layout.addWidget(self._moon_btn)

        self._yolo_btn = self._make_rail_button("🤖\nYOLO")
    ...
    def _wire_signals(self) -> None:
        # Queued connections (ROS thread -> GUI thread).
    ...
    def _on_status(self, status: dict) -> None:
        ...
        self._status_label.setText(f'<span style="color:{color}">●</span> ROS: {text}')
        temp = status.get("temperature_c")
        self._temp_label.setText(f"T: {temp:.1f}°C" if temp is not None else "T: –")

    def _on_error(self, message: str) -> None:
        self._status_label.setText(f'<span style="color:#f87171">●</span> {message}')

    # -- UI actions -----------------------------------------------------------
    def _select_tool(self, tool: str) -> None:
        if tool == self._active_tool and not self._panel_collapsed:
            # Tapping the active tool collapses the panel -> preview full width.
    ...
    def _toggle_fullscreen(self, fullscreen: bool | None = None) -> None:
        """Enter or leave fullscreen, keeping the top-bar button in sync.

        ``fullscreen=None`` means "flip", which is what both the button and F11
        want; ``showNormal`` restores the pre-fullscreen geometry by itself.
        """
    ...
    def changeEvent(self, event) -> None:  # noqa: N802
        # main.py starts the app with showFullScreen() on the Pi, and the window
        # manager can change state on its own, so track the real window state
        # instead of only the paths that go through _toggle_fullscreen. showEvent
        # is too early to see the fullscreen flag; WindowStateChange is not.
    ...
    def _capture(self) -> None:
        ...
        try:
            ...
            self._status_label.setText(
                f'<span style="color:#4ade80">●</span> saved {path.name}'
            )
